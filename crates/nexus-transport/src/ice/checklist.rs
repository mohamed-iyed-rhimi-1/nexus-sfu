...
pub struct Checklist {
    ...
    /// Next pair index to check.
    #[allow(dead_code)] // Reserved for sequential check scheduling
    next_check_idx: u16,
    
    /// Use aggressive nomination (RFC 8445).
    #[allow(dead_code)] // Reserved for aggressive nomination mode
...
impl Checklist {
    ...
    pub fn form_pairs(
        ...
        assert!(local_candidates.len() <= MAX_CANDIDATES,
            "Local candidates must be <= MAX_CANDIDATES");
        assert!(remote_candidates.len() <= MAX_CANDIDATES,
        ...
        for local in local_candidates {
            for remote in remote_candidates {
                ...
                assert!((self.pair_count as usize) < MAX_CANDIDATE_PAIRS,
        ...
        assert!(self.pair_count <= MAX_CANDIDATE_PAIRS as u16,
    ...
    pub fn add_pair(
        ...
        assert_eq!(self.pair_count, count_before + 1,
    ...
    fn sort_pairs(&mut self) {
        ...
        for i in 1..self.pair_count as usize {
            ...
            assert!(i < self.pair_count as usize,
            ...
            while j > 0 {
                ...
                assert!(j < i + 1,
                    "Inner loop index must be bounded");
    ...
    fn prune_pairs(&mut self) {
        ...
        assert!(self.pair_count > 0,
            "Prune called with valid pair count");
        ...
        for i in 0..self.pair_count as usize {
            ...
            for j in 0..i {
                ...
                assert!(j < i,
                    "Inner loop index must be less than outer");
        ...
        assert!(valid_count <= self.pair_count,
    ...
    pub fn next_check(&mut self) -> Option<(u16, SocketAddr, [u8; STUN_BUFFER_SIZE], usize)> {
        ...
        assert!(self.state == ChecklistState::Running,
        ...
        assert!(self.pending_count < MAX_PARALLEL_CHECKS as u8,
    ...
    fn create_check_request(&self, pair_idx: usize, nominating: bool) -> ([u8; STUN_BUFFER_SIZE], usize) {
        ...
        let username = format!(
            "{}:{}",
            self.remote_credentials.local_ufrag,
            self.local_credentials.local_ufrag
        );
    ...
    pub fn process_response(
        ...
        assert!(self.pairs[pair_idx].is_some(),
        ...
        match msg.class {
            ...
            StunClass::ErrorResponse => {
                ...
                let error_code = msg.attributes.iter()
                    .flatten()
                    .find_map(|attr| {
                        if let crate::ice::stun::StunAttribute::ErrorCode { code, reason, reason_len } = attr {
                            let reason_str = std::str::from_utf8(&reason[..*reason_len as usize]).unwrap_or("?");
    ...
    fn handle_success_response(
        ...
        assert!(self.pairs[pair_idx].as_ref().map(|p| p.state) == Some(CandidatePairState::Succeeded),
    ...
    pub fn handle_use_candidate_request(
        &mut self,
        from: SocketAddr,
    ) -> bool {
    ...
    pub fn check_retransmissions(&mut self) -> Vec<(u16, SocketAddr, [u8; STUN_BUFFER_SIZE], usize)> {
        ...
        for (slot_idx, slot) in self.pending.iter().enumerate() {
            ...
            assert!(slot_idx < MAX_PARALLEL_CHECKS,
                "Slot index must be within bounds");
            
            if let Some(ref check) = slot {
                // Retry count assertion (TigerStyle Phase 3.7)
                assert!(check.retransmissions <= MAX_RETRANSMISSIONS as u8,
        ...
        assert!(retransmits.len() <= MAX_PARALLEL_CHECKS,
    ...
    pub fn nominate(&mut self) -> Option<(u16, SocketAddr, [u8; STUN_BUFFER_SIZE], usize)> {
        ...
        assert!(matches!(self.role, IceRole::Controlling),
        ...
        if let Some(idx) = best_idx {
            ...
            if let Some(ref pair) = self.pairs[idx] {
                ...
                assert!(self.nominated_idx.is_none() || best_idx.is_some(),
        ...
        assert!(self.nominated_idx.is_some() || best_idx.is_none() || best_idx.is_some(),
    ...
    pub fn nominated_pair(&self) -> Option<&CandidatePair> {
        self.nominated_idx.and_then(|idx| self.pairs[idx as usize].as_ref())
    }
    ...
    pub fn checked_count(&self) -> u16 {
        ...
        for i in 0..self.pair_count as usize {
            if let Some(ref pair) = self.pairs[i] {
                if pair.state == CandidatePairState::Succeeded || pair.state == CandidatePairState::Failed {
                    count += 1;
                }
            }
        }
...
mod tests {
    ...
    fn test_pair_priority_calculation() {
        ...
        assert!(pair.priority < u64::MAX / 2, "Pair priority should be bounded");
    }
    ...
    fn test_controlling_vs_controlled_priority() {
        ...
        let mut checklist_ctld = Checklist::new(
            local_creds, 
            remote_creds, 
            IceRole::Controlled
        );
    ...
    fn test_max_candidate_pairs_constant() {
        assert_eq!(MAX_CANDIDATE_PAIRS, 100, 
    ...
    fn test_pair_list_bounds_enforcement() {
        ...
        assert!(checklist.pair_count() as usize <= MAX_CANDIDATE_PAIRS,
            "Pair count {} exceeds MAX_CANDIDATE_PAIRS {}", 
            checklist.pair_count(), MAX_CANDIDATE_PAIRS);
    }
    ...
    fn test_max_parallel_checks_constant() {
        assert_eq!(MAX_PARALLEL_CHECKS, 5,
            "MAX_PARALLEL_CHECKS should be 5");
    }
    ...
    fn test_different_checklists_different_tie_breakers() {
        ...
        let checklist1 = Checklist::new(local_creds.clone(), remote_creds.clone(), IceRole::Controlling);
    ...
    fn test_retransmission_constants() {
        ...
        assert_eq!(MAX_RETRANSMISSIONS, 7,
            "Max retransmissions should be 7");
